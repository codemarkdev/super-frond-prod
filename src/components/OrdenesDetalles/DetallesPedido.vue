  ...
  <div class="detalles-pedido">
        ...
        <form @submit.prevent="crearPedido" class="form">
          ...
          <div class="form-group">
            ...
            <div class="select-wrapper">
              <select id="providerId" v-model="nuevoPedido.providerId" class="form-control" required>
            ...
            <button v-if="proveedores.length === 0" type="button" @click="fetchProveedores" class="btn btn-small">
          ...
          <div class="form-group">
            ...
            <div class="input-wrapper">
              <input type="date" id="orderDate" v-model="nuevoPedido.orderDate" class="form-control" required />
            </div>
          </div>
          ...
          <div class="form-group">
            ...
            <div class="input-wrapper">
              <input type="text" id="invoiceNumber" v-model="nuevoPedido.invoiceNumber" class="form-control"
    ...
    <div class="section">
      ...
      <div v-show="isPedidosOpen" class="section-content">
        ...
        <div class="search-container">
          <i class="fas fa-search search-icon"></i>
          <input type="text" v-model="searchPedidoQuery" placeholder="Buscar por número de factura..."
            class="search-input" />
          ...
          <div class="filter-container">
            ...
            <input type="date" v-model="fechaFiltro" class="date-filter" />
        ...
        <div v-else-if="filteredPedidos.length === 0" class="message">
          <i class="fas fa-info-circle message-icon"></i>
          <span>No se encontraron pedidos{{ searchPedidoQuery || fechaFiltro ? ' con los filtros aplicados' : ''
          }}.</span>
        </div>
        ...
        <div v-else class="pedidos-container">
          <div v-for="pedido in filteredPedidos" :key="pedido.id" class="pedido-card"
            :class="{ 'expanded': expandedCards[pedido.id] }">
            ...
            <div v-show="expandedCards[pedido.id]" class="pedido-footer">
              <div class="pedido-actions">
                <button class="btn-action btn-view" @click="verDetallesPedido(pedido.id)">
      ...
      <div class="modal-container details-modal">
        ...
        <div class="modal-body">
          ...
          <div v-else class="pedido-details-container">
            ...
            <div class="details-section" v-if="pedidoDetallado.orderDetails && pedidoDetallado.orderDetails.length > 0">
              <h4 class="details-section-title">Productos</h4>
              <div class="details-table">
                <div class="details-table-header">
                  <div class="details-table-cell">ID</div>
                  <div class="details-table-cell">Cantidad</div>
                  <div class="details-table-cell">Precio Unitario</div>
                  <div class="details-table-cell">Impuesto Unitario</div>
                  <div class="details-table-cell">Subtotal</div>
                  <div class="details-table-cell">Total Impuesto</div>
                  <div class="details-table-cell">Total</div>
                </div>
                <div v-for="detalle in pedidoDetallado.orderDetails" :key="detalle.id" class="details-table-row">
                  <div class="details-table-cell">{{ detalle.id }}</div>
                  <div class="details-table-cell">{{ detalle.quantity }}</div>
                  <div class="details-table-cell">{{ formatearMoneda(detalle.purchasePriceUnit) }}</div>
                  <div class="details-table-cell">{{ formatearMoneda(detalle.calculatedTaxUnit) }}</div>
                  <div class="details-table-cell">{{ formatearMoneda(detalle.calculatedTotalPriceWithouthTax) }}</div>
                  <div class="details-table-cell">{{ formatearMoneda(detalle.calculatedTotalTax) }}</div>
                  <div class="details-table-cell total-cell">{{ formatearMoneda(calcularTotalDetalle(detalle)) }}</div>
                </div>
              </div>
              ...
              <div class="details-totals">
                <div class="details-total-row">
                  ...
                  <div class="details-total-value">{{ formatearMoneda(calcularSubtotalPedido(pedidoDetallado)) }}</div>
                </div>
                <div class="details-total-row">
                  <div class="details-total-label">Total Impuestos:</div>
                  <div class="details-total-value">{{ formatearMoneda(calcularTotalImpuestosPedido(pedidoDetallado)) }}
                  </div>
                </div>
                <div class="details-total-row grand-total">
                  <div class="details-total-label">Total General:</div>
                  <div class="details-total-value">{{ formatearMoneda(calcularTotalGeneralPedido(pedidoDetallado)) }}
                  </div>
                </div>
              </div>
            </div>
        ...
        <div class="modal-body">
          <form @submit.prevent="actualizarPedido" class="form">
            ...
            <div class="form-group">
              ...
              <div class="input-wrapper">
                <input type="date" id="editOrderDate" v-model="pedidoEditando.orderDate" class="form-control"
                  required />
              </div>
            </div>
            ...
            <div class="form-group">
              ...
              <div class="input-wrapper">
                <input type="text" id="editInvoiceNumber" v-model="pedidoEditando.invoiceNumber" class="form-control"
                  placeholder="Ej: INV-12345" required />
              </div>
            </div>
      ...
      <div class="modal-container delete-modal">
        ...
        <div class="modal-body">
          ...
          <div class="form-actions delete-actions">
            ...
            <button type="button" @click="eliminarPedido" class="btn btn-danger" :disabled="eliminandoPedido">
...
import '@/components/stilos/detalles-pedido.css';
...
export default {
  ...
  methods: {
    ...
    // Nuevo método para ver detalles completos de un pedido
    async verDetallesPedido(id) {
